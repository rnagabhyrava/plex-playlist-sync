...
while True:
    ...
    if SPAUTHSUCCESS:
        ...
        if not sp_playlists:
            ...
            for playlist, name in sp_playlists:
                track_names, poster_url, playlist_description = get_sp_track_names(
                ...
                create_plex_playlist(plex, tracksList=trackList,
                                     playlistName=name+" - Spotify", posterURL=poster_url, description=playlist_description)
    ...
    if not dz_playlists:
        ...
        for playlist, name in dz_playlists:
            tracks, poster_url, playlist_description = get_dz_playlist_tracks(
                dz, playlist)
            trackList = get_available_plex_tracks(plex, tracks)
            create_plex_playlist(plex, tracksList=trackList,
                                 playlistName=name+" - Deezer", posterURL=poster_url, description=playlist_description)
